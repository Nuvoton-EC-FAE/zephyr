...
LOG_MODULE_REGISTER(host_sub_npcx, LOG_LEVEL_INF);
...
struct host_sub_npcx_config {
	...
	struct pmch_reg *const inst_acpi_pm3;
	struct pmch_reg *const inst_acpi_pm4;
	/* clock configuration */
...
#define NPCX_EC_CFG_IO_ADDR		CONFIG_EC_HOST_CFG_BASE_ADDR

/* Timeout to wait for Core-to-Host transaction to be completed. */
#define NPCX_C2H_TRANSACTION_TIMEOUT_US 200

/* the host device enum that accessed through host module internal bus */
enum hmib_device_type {
	HMIB_DEV_CONFIG = 0,
	HMIB_DEV_SERIAL2 = 1,
	HMIB_DEV_SERIAL = 2,
	HMIB_DEV_ESHM = 3,
...
#define EC_CFG_LDN_PM3   0x17 /* PM Channel 3 */
#define EC_CFG_LDN_PM4   0x1E /* PM Channel 4 */
...
#define EC_CFG_IDX_SHM_DP80_ADDR_SL    0xFC
#define EC_CFG_IDX_SHM_DP80_ADDR_RANGE 0xFD
#define EC_CFG_IDX_SHM_DP80_ADDR_L     0xFE
#define EC_CFG_IDX_SHM_DP80_ADDR_H     0xFF
...
#define EC_CFG_SHM_CFG_WRAM1_IO        0x10
#define EC_CFG_SHM_CFG_WRAM2_IO        0x20
#define EC_CFG_ESHM_CFG_WRAM3_IO       0x10
#define EC_CFG_ESHM_CFG_WRAM4_IO       0x20
...
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_SIZE)
static uint8_t shm_wnd1_mmap[CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_SIZE] 
	__aligned(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_SIZE);
#endif

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_SIZE)
static uint8_t shm_wnd2_mmap[CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_SIZE] 
	__aligned(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_SIZE);
#endif

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_SIZE)
static uint8_t shm_wnd3_mmap[CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_SIZE] 
	__aligned(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_SIZE);
#endif

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_SIZE)
static uint8_t shm_wnd4_mmap[CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_SIZE] 
	__aligned(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_SIZE);
#endif

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1) || defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2) || \
	defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3) || defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4)

static void host_shared_mem_region_init(void)
{
	struct shm_reg *const inst_shm = host_sub_cfg.inst_shm;

	LOG_INF("host shared memory region init");

	/* Don't stall SHM transactions */
	inst_shm->SHM_CTL &= ~BIT(NPCX_SHM_CTL_STALL_HOST);

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1)	
	...
	inst_shm->HOFS_CTL &= ~BIT(NPCX_HOFS_CTL_HOFS1R_IE);
	/* enable interrupt by host write to offset in window 1 */
	inst_shm->HOFS_CTL |= BIT(NPCX_HOFS_CTL_HOFS1W_IE);
	/* disable interrupt by host semaphore 1 writtern */
	inst_shm->SMC_CTL &= ~BIT(NPCX_SMC_CTL_HSEM1_IE);
#endif

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2)
	/* Disable Window 2 protection */
	inst_shm->WIN2_WR_PROT = 0;
	inst_shm->WIN2_RD_PROT = 0;

	/* Configure Win2 size for ec host command. */
	...
	inst_shm->HOFS_CTL &= ~BIT(NPCX_HOFS_CTL_HOFS2R_IE);
	/* enable interrupt by host write to offset in window 2 */
	inst_shm->HOFS_CTL |= BIT(NPCX_HOFS_CTL_HOFS2W_IE);
	/* disable interrupt by host semaphore 2 writtern */
	inst_shm->SMC_CTL &= ~BIT(NPCX_SMC_CTL_HSEM2_IE);
#endif

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3)
	...
	inst_shm->HOFSE_CTL |= BIT(NPCX_HOFSE_CTL_HOFS3W_IE);
	/* disable interrupt by host semaphore 3 writtern */
	inst_shm->SMCE_CTL &= ~BIT(NPCX_SMCE_CTL_HSEM3_IE);
#endif

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4)
	...
	/* enable interrupt by host write to offset in window 4 */
	inst_shm->HOFSE_CTL |= BIT(NPCX_HOFSE_CTL_HOFS4W_IE);
	/* disable interrupt by host semaphore 4 writtern */
	inst_shm->SMCE_CTL &= ~BIT(NPCX_SMCE_CTL_HSEM4_IE);
#endif
}
#endif
...
#if defined(ESPI_PERIPHERAL_SHARED_WINDOW_1) || defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2) || \
	defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3) || defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4)

static void host_shm_isr(const void *arg)
{
	ARG_UNUSED(arg);
	struct shm_reg *const inst_shm = host_sub_cfg.inst_shm;
	uint8_t smc_sts = inst_shm->SMC_STS;
	uint8_t hofs_sts = inst_shm->HOFS_STS;
    uint8_t smce_sts  = inst_shm->SMCE_STS;
    uint8_t hofse_sts = inst_shm->HOFSE_STS;

	struct espi_event evt = {
		.evt_type = ESPI_BUS_PERIPHERAL_NOTIFICATION,
		.evt_details = ESPI_PERIPHERAL_SHARED_MEMORY,
		.evt_data = ESPI_PERIPHERAL_NODATA
	};

	/* Clear all pending bit indicates that FIFO was written by host */
	inst_shm->SMC_STS = smc_sts;
	inst_shm->HOFS_STS = hofs_sts;
	inst_shm->SMCE_STS = smce_sts;
	inst_shm->HOFSE_STS = hofse_sts;

	if (IS_BIT_SET(smc_sts, NPCX_SMC_STS_HSEM1W))
    {
		evt.evt_details = ESPI_PERIPHERAL_SHARED_MEMORY |
						  (SHM_WINDOW_ONE << SHM_EVENT_CHANNEL_POS) |
						  (SHM_HOST_SEMAPHORE_WRITTEN << SHM_EVENT_TYPE_POS);

		/* Send data to the callback function for further processing */
		espi_send_callbacks(host_sub_data.callbacks, host_sub_data.host_bus_dev, evt);
	}

 	if (IS_BIT_SET(hofs_sts, NPCX_HOFS_STS_HOFS1W))
	{
		evt.evt_details = ESPI_PERIPHERAL_SHARED_MEMORY |
						  (SHM_WINDOW_ONE << SHM_EVENT_CHANNEL_POS) |
						  (SHM_HOST_WRITE_TO_OFFSET << SHM_EVENT_TYPE_POS);

	  	/* Send data to the callback function for further processing */
		espi_send_callbacks(host_sub_data.callbacks, host_sub_data.host_bus_dev, evt);
    }

	if (IS_BIT_SET(smc_sts, NPCX_SMC_STS_HSEM2W))
	{
		evt.evt_details = ESPI_PERIPHERAL_SHARED_MEMORY |
						  (SHM_WINDOW_TWO << SHM_EVENT_CHANNEL_POS) |
						  (SHM_HOST_SEMAPHORE_WRITTEN << SHM_EVENT_TYPE_POS);
	...
	if (IS_BIT_SET(hofs_sts, NPCX_HOFS_STS_HOFS2W))
    {
		evt.evt_details = ESPI_PERIPHERAL_SHARED_MEMORY |
						  (SHM_WINDOW_TWO << SHM_EVENT_CHANNEL_POS) |
						  (SHM_HOST_WRITE_TO_OFFSET << SHM_EVENT_TYPE_POS);
	...
	if (IS_BIT_SET(smce_sts, NPCX_SMCE_STS_HSEM3W))
	{
		evt.evt_details = ESPI_PERIPHERAL_SHARED_MEMORY |
						  (SHM_WINDOW_THREE << SHM_EVENT_CHANNEL_POS) |
						  (SHM_HOST_SEMAPHORE_WRITTEN << SHM_EVENT_TYPE_POS);
	...
	if (IS_BIT_SET(hofse_sts, NPCX_HOFSE_STS_HOFS3W))
    {
		evt.evt_details = ESPI_PERIPHERAL_SHARED_MEMORY |
						  (SHM_WINDOW_THREE << SHM_EVENT_CHANNEL_POS) |
						  (SHM_HOST_WRITE_TO_OFFSET << SHM_EVENT_TYPE_POS);
	...
	if (IS_BIT_SET(smce_sts, NPCX_SMCE_STS_HSEM4W))
	{
		evt.evt_details = ESPI_PERIPHERAL_SHARED_MEMORY |
						  (SHM_WINDOW_FOUR << SHM_EVENT_CHANNEL_POS) |
						  (SHM_HOST_SEMAPHORE_WRITTEN << SHM_EVENT_TYPE_POS);
	...
	if (IS_BIT_SET(hofse_sts, NPCX_HOFSE_STS_HOFS4W))
    {
		evt.evt_details = ESPI_PERIPHERAL_SHARED_MEMORY |
						  (SHM_WINDOW_FOUR << SHM_EVENT_CHANNEL_POS) |
						  (SHM_HOST_WRITE_TO_OFFSET << SHM_EVENT_TYPE_POS);
...
{
	LOG_DBG("%s: enable host sub-device interrupts", __func__);
	...
	/* Enable host shared memory device interrupt */
	if (IS_ENABLED(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1) ||
		IS_ENABLED(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2) ||
		IS_ENABLED(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3) ||
		IS_ENABLED(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4)) {
		irq_enable(DT_INST_IRQ_BY_NAME(0, shm, irq));
	}
...
uint8_t host_c2h_read_reg(enum hmib_device_type device, uint8_t offset)
{
	struct c2h_reg *const inst_c2h = host_sub_cfg.inst_c2h;
	uint8_t data_val;
	...
	/* Select the module to be accessed, if not already set. */
	inst_c2h->CRSMAE |= BIT(device);
	/* Specify the offset of the register from the module base address */
	...
	/* Disable Core-to-Host access module */
	inst_c2h->CRSMAE &= ~BIT(device);
	/* Unlock host access module registers */
	inst_c2h->LKSIOHA &= ~BIT(device);
...
void host_c2h_write_reg(enum hmib_device_type device, uint8_t offset, uint8_t data)
{
	struct c2h_reg *const inst_c2h = host_sub_cfg.inst_c2h;
								...
								uint32_t *data)
{
	if (op >= E8042_START_OPCODE && op <= E8042_MAX_OPCODE) {
		...
		switch (op) {
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_SIZE)
		case ECUSTOM_SHD_WND1_GET_MEMORY:
			*data = (uint32_t)shm_wnd1_mmap;
			break;
		case ECUSTOM_SHD_WND1_GET_MEMORY_SIZE:
			*data = CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_SIZE;
			break;
#endif
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_SIZE)
		case ECUSTOM_SHD_WND2_GET_MEMORY:
			*data = (uint32_t)shm_wnd2_mmap;
			break;
		case ECUSTOM_SHD_WND2_GET_MEMORY_SIZE:
			*data = CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_SIZE;
			break;
#endif
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_SIZE)
		case ECUSTOM_SHD_WND3_GET_MEMORY:
			*data = (uint32_t)shm_wnd3_mmap;
			break;
		case ECUSTOM_SHD_WND3_GET_MEMORY_SIZE:
			*data = CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_SIZE;
			break;
#endif
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_SIZE)
		case ECUSTOM_SHD_WND4_GET_MEMORY:
			*data = (uint32_t)shm_wnd4_mmap;
			break;
		case ECUSTOM_SHD_WND4_GET_MEMORY_SIZE:
			*data = CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_SIZE;
			break;
#endif	
		default:
			return -EINVAL;
...
{
	struct c2h_reg *const inst_c2h = host_sub_cfg.inst_c2h;
	uint8_t shm_cfg1 = 0x00;

	LOG_DBG("%s: init host sub-device in host domain", __func__);
...
#if defined(CONFIG_ESPI_PERIPHERAL_HOST_IO_PORT_NUM)
		/* Configure IO address of CMD portt */
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_CMD_IO_ADDR_H,
		 (CONFIG_ESPI_PERIPHERAL_HOST_IO_PORT_NUM >> 8) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_CMD_IO_ADDR_L,
		 CONFIG_ESPI_PERIPHERAL_HOST_IO_PORT_NUM & 0xff);
		/* Configure IO address of Data portt */
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_DATA_IO_ADDR_H,
		 ((CONFIG_ESPI_PERIPHERAL_HOST_IO_PORT_NUM + 4) >> 8) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_DATA_IO_ADDR_L,
		 (CONFIG_ESPI_PERIPHERAL_HOST_IO_PORT_NUM + 4) & 0xff);
#endif
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_CTRL, EC_CFG_IDX_CTRL_LDN_ENABLE);
	}
...
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_CFG_PORT_NUM)
		/* Configure IO address of CMD port */
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_CMD_IO_ADDR_H,
		 (CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_CFG_PORT_NUM >> 8) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_CMD_IO_ADDR_L,
		 CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_CFG_PORT_NUM & 0xff);
#endif

		/* WIN 1 & 2 mapping to IO space */
		shm_cfg1 = host_c2h_read_io_cfg_reg(EC_CFG_IDX_SHM_CFG);
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_IO)
		shm_cfg1 |= EC_CFG_SHM_CFG_WRAM1_IO;
#else
		shm_cfg1 &= ~EC_CFG_SHM_CFG_WRAM1_IO;
#endif

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_IO)
		shm_cfg1 |= EC_CFG_SHM_CFG_WRAM2_IO;
#else
		shm_cfg1 &= ~EC_CFG_SHM_CFG_WRAM2_IO;
#endif

		host_c2h_write_io_cfg_reg(EC_CFG_IDX_SHM_CFG, shm_cfg1);

		/* WIN1 as Host Command on the base address */
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_PORT_NUM)
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_SHM_WND1_ADDR_3,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_PORT_NUM >> 24) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_SHM_WND1_ADDR_2,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_PORT_NUM >> 16) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_SHM_WND1_ADDR_1,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_PORT_NUM >> 8) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_SHM_WND1_ADDR_0,
		CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1_PORT_NUM & 0xff);
#endif
		/* Set WIN2 as MEMMAP on the configured IO address */
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_PORT_NUM)
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_SHM_WND2_ADDR_3,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_PORT_NUM >> 24) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_SHM_WND2_ADDR_2,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_PORT_NUM >> 16) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_SHM_WND2_ADDR_1,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_PORT_NUM >> 8) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_SHM_WND2_ADDR_0,
		CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2_PORT_NUM & 0xff);
#endif
	...
	if (IS_ENABLED(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3) ||
	    IS_ENABLED(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4)) {
		/* Select 'Shared Memory' bank which LDN are 0x1D */
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_LDN, EC_CFG_LDN_ESHM);

#if defined(CONFIG_ESPI_PERIPHERAL_EXTENDED_SHARED_WINDOW_CFG_PORT_NUM)
		/* Configure IO address of CMD port */
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_CMD_IO_ADDR_H,
		 (CONFIG_ESPI_PERIPHERAL_EXTENDED_SHARED_WINDOW_CFG_PORT_NUM >> 8) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_CMD_IO_ADDR_L,
		 CONFIG_ESPI_PERIPHERAL_EXTENDED_SHARED_WINDOW_CFG_PORT_NUM & 0xff);
#endif

		/* WIN 3 & 4 mapping to IO space */
		shm_cfg1 = host_c2h_read_io_cfg_reg(EC_CFG_IDX_ESHM_CFG);
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_IO)
		shm_cfg1 |= EC_CFG_ESHM_CFG_WRAM3_IO;
#else
		shm_cfg1 &= ~EC_CFG_ESHM_CFG_WRAM3_IO;
#endif

#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_IO)
		shm_cfg1 |= EC_CFG_ESHM_CFG_WRAM4_IO;
#else
		shm_cfg1 &= ~EC_CFG_ESHM_CFG_WRAM4_IO;
#endif

		host_c2h_write_io_cfg_reg(EC_CFG_IDX_ESHM_CFG, shm_cfg1);

		/* WIN3 as Host Command on the base address */
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_PORT_NUM)
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_ESHM_WND3_ADDR_3,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_PORT_NUM >> 24) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_ESHM_WND3_ADDR_2,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_PORT_NUM >> 16) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_ESHM_WND3_ADDR_1,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_PORT_NUM >> 8) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_ESHM_WND3_ADDR_0,
		CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3_PORT_NUM & 0xff);
#endif
		/* Set WIN4 as MEMMAP on the configured IO address */
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_PORT_NUM)
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_ESHM_WND4_ADDR_3,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_PORT_NUM >> 24) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_ESHM_WND4_ADDR_2,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_PORT_NUM >> 16) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_ESHM_WND4_ADDR_1,
		(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_PORT_NUM >> 8) & 0xff);
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_ESHM_WND4_ADDR_0,
		CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4_PORT_NUM & 0xff);
#endif

		/* Enable ESHM direct memory access */
		host_c2h_write_io_cfg_reg(EC_CFG_IDX_CTRL, EC_CFG_IDX_CTRL_LDN_ENABLE);
	}
							...
							sys_slist_t *callbacks)
{
	...
	LOG_DBG("%s: init host sub-device in core domain", __func__);
	...
	/* Set host access shm response and disable host access and host access error interrupt */
	SET_FIELD(inst_shm->SMC_CTL, NPCX_SMC_CTL_HERES, 0);
	inst_shm->SMC_CTL &= ~(BIT(NPCX_SMC_CTL_HERR_IE) | BIT(NPCX_SMC_CTL_ACC_IE));
	...
	/* Host shared memory sub-device interrupt installation */
#if defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_1) || \
	defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_2) || \
	defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_3) || \
	defined(CONFIG_ESPI_PERIPHERAL_SHARED_WINDOW_4)	
	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(0, shm, irq),
		    DT_INST_IRQ_BY_NAME(0, shm, priority),
		    host_shm_isr,
		    NULL, 0);
#endif
